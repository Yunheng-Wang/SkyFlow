...
'''
作用：初始化多个检测区域（PolygonZone）
参数：
    polygons -> 由四个坐标点组成的多边形区域列表
    triggering_anchors -> 指定目标的哪个位置触发进入区域的检测（默认中心点）
返回：
    返回一个包含多个 PolygonZone 对象的列表
'''
...
class VideoProcessor:
    # -> None ： 表示返回为none
    def __init__(self, source_weights_path: str, 
    source_video_path: str, 
    target_video_path: Optional[str] = None, 
    confidence_threshold: float = 0.3,  # yolo模型的一个超参数，目标置信度阈值
    iou_threshold: float = 0.7          # yolo模型的一个超参数，去重叠阈值
    ,)-> None:
